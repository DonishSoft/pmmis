    ...
    public async Task<IActionResult> UploadExcel(int contractId, IFormFile file, string mode = "ClosedXML")
    {
        try
        {
            ...
            return Json(new
            {
                ...
                matchedList = result.Matched.Select(m => new { m.ExistingId, m.Name, m.Category, m.Unit, m.TargetQuantity, m.ThisPeriodQuantity, m.ThisPeriodAmount, m.PreviousQuantity, m.PreviousAmount })
            });
        }
        catch (Exception ex)
        {
